...
#                            01/12/2022 18:39:54                               #
#                  *  Poisson-eps[individual,trap,occasion] *                  #
#                                                                              #
#==============================================================================#
...
code <- nimbleCode({
  ...
  for(i in 1:M){
    for(j in 1:J){
      for(k in 1:K){
        lp[i,j,k] ~ dnorm(mu0, sd=sigma.p)
        log(lam0[i,j,k]) <- lp[i,j,k]
      }
    }
  }
  ...
  for(i in 1:M){
    ...
    for(j in 1:J){
      ...
      for(k in 1:K){  
        lam[i,j,k] <- lam0[i,j,k]*exp(-alpha1*d2[i,j])
        mu[i,j,k] <- lam[i,j,k]*z[i]
        Y[i,j,k] ~ dpois(mu[i,j,k]) 
        # components for fit diagnostics
        Ysim[i,j,k] ~ dpois(mu[i,j,k])       # simulated
        Yexp[i,j,k] <- lam[i,j,k]*z[i]       # expected
      }
      # components for T1
      err1obs[i,j] <- (sqrt(sum(Y[i,j,1:K])) - sqrt(sum(Yexp[i,j,1:K])))^2
      err1sim[i,j] <- (sqrt(sum(Ysim[i,j,1:K])) - sqrt(sum(Yexp[i,j,1:K])))^2
    }      
    ...
    # components for T2
    err2obs[i] <- (sqrt(sum(Y[i,1:J,1:K])) - sqrt(sum(Yexp[i,1:J,1:K])))^2
    err2sim[i] <- (sqrt(sum(Ysim[i,1:J,1:K])) - sqrt(sum(Yexp[i,1:J,1:K])))^2
  }
  
  # components for T3
  for(j in 1:J){
    err3obs[j] <- (sqrt(sum(Y[1:M,j,1:K])) - sqrt(sum(Yexp[1:M,j,1:K])))^2
    err3sim[j] <- (sqrt(sum(Ysim[1:M,j,1:K])) - sqrt(sum(Yexp[1:M,j,1:K])))^2
  }
...
mcmc<-configureMCMC(Rmodel,  monitors=params, enableWAIC = TRUE, thin = 1) 
